...
import { getHelmInstallStdOptions, SumoHelmStdOptions } from '../../helm/fresh/freshInstallHelm';
...
const assertFluentdAndEventsPreconditions = (opts: SumoHelmStdOptions) => {
...
const setupFluentdAndEvents = async (answers: SumoHelmStdOptions) => {
    console.log("Setting up SumoLogic collectors and fluentd and fluentd-events");
    assertFluentdAndEventsPreconditions(answers);
    const proceed = await setupQuestionsFactory(questionGenerator)("Is it okay to download the `setup.sh` script as `sumo-setup.sh` from github? [Y/n]");
    if (!proceed) {
    ...
    const proceedWithCmd = await setupQuestionsFactory(questionGenerator)(`Proceed to run cmd: ${cmd}? [Y/n]`);
    if (!proceedWithCmd) {
        process.exit(0);
    }
...
const setupPrometheusOperator = async (answers: SumoHelmStdOptions) => {
    console.log("Setting up Prometheus Operator");
    assertPrometheusOperatorPreconditions();
    const proceed = await setupQuestionsFactory(questionGenerator)("Is it okay to download the `prometheus-overrides.yaml` from github? [Y/n]");
    if (!proceed) {
        process.exit(0);
    }
    ...
    const proceedToApply = await setupQuestionsFactory(questionGenerator)("Proceed to run `kubectl apply -f prometheus.yaml`? [Y/n]");
    if (!proceedToApply) {
        process.exit(0);
    }
...
const setupFluentBit = async (answers: SumoHelmStdOptions) => {
    console.log("Setting up fluent-bit");
    const proceed = await setupQuestionsFactory(questionGenerator)("Is it okay to download the `fluent-bit-overrides.yaml` from github? [Y/n]");
    if (!proceed) {
        process.exit(0);
    }
    ...
    const fluentBitTgz: string = shell.exec("ls | grep fluent-bit.*.tgz").trim();
    ...
    const proceedCmd = await setupQuestionsFactory(questionGenerator)(`Cmd: ${cmd}. [Y/n]`);
    if (!proceedCmd) {
        process.exit(0);
    }
    shell.exec(cmd);
    ...
    const proceedToApply = await setupQuestionsFactory(questionGenerator)(`Proceed to run ${applyCmd}? [Y/n]`);
    if (!proceedToApply) {
        process.exit(0);
    }
    shell.exec(applyCmd);
...
const setupFalco = async (answers: SumoHelmStdOptions) => {
    console.log("Setting up Falco!");
    const proceed = await setupQuestionsFactory(questionGenerator)("Is it okay to download the `falco-overrides.yaml` from github? [Y/n]");
    if (!proceed) {
        process.exit(0);
    }
    ...
    const cmdProceed = await setupQuestionsFactory(questionGenerator)(`Cmd: ${cmd}. [Y/n]`);
    if (!cmdProceed) {
        process.exit(0);
    }
...
const setupPrometheusRules = async (answers: SumoHelmStdOptions) => {
...
const main = async () => {
    ...
    const proceed = await setupQuestionsFactory(questionGenerator)('Do you want to proceed with the non-helm installation? [Y/n]');
    if (!proceed) {
        process.exit(0);
    }

    const answers = await getHelmInstallStdOptions();
    const installFluentd = await setupQuestionsFactory(questionGenerator)("Do you want to install sumo collectors, fluentd, and fluentd-events? [Y/n]");
    if (installFluentd) {
        await setupFluentdAndEvents(answers);
    }

    const installFluentBit = await setupQuestionsFactory(questionGenerator)("Do you want to install fluent-bit? [Y/n]");
    if (installFluentBit) {
        await setupFluentBit(answers);
    }

    const installPrometheusOperator = await setupQuestionsFactory(questionGenerator)("Do you want to install a new prometheus operator? [Y/n]");
    if (installPrometheusOperator) {
        await setupPrometheusOperator(answers);
    }

    const installFalco = await setupQuestionsFactory(questionGenerator)("Do you want to install Faclo? [Y/n]");
    if (installFalco) {
        await setupFalco(answers);
    }

    const installPrometheusRules = await setupQuestionsFactory(questionGenerator)("Do you want to install the additional Prometheus Rules? [Y/n]");
    if (installPrometheusRules) {
        await setupPrometheusRules(answers);
    }
}

export { main as nonHelmInstall };