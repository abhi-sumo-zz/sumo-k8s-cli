...
const confirmHelmInstall = async () => question_1.setupQuestionsFactory(questionGenerator)('This will install sumo logic collection with Helm. Are you sure you want to proceed? [Y/n]');
const installSumoHelmRepo = async () => {
    console.log("Starting the helm install process...");
    const proceed = await question_1.setupQuestionsFactory(questionGenerator)("Cmd: `helm repo add sumologic https://sumologic.github.io/sumologic-kubernetes-collection` Proceed? [Y/n]");
    if (proceed) {
...
const isValuesFileBasedInstall = async () => {
    const answer = await question_1.setupQuestionsFactory(questionGenerator)("Will you be providing a values.yaml file? [Y/n]");
    return answer;
};
const verifyStdOpts = (stdOpts) => {
    console.log('TODO: verficiation function needs to be written');
};
const getHelmInstallStdOptions = async () => {
...
exports.getHelmInstallStdOptions = getHelmInstallStdOptions;
const assertStdReleaseInstallPreconditions = (opts) => {
...
const addValuesOpt = async (opts) => {
    if (opts.releaseName !== 'collection' && opts.namespace !== 'sumologic') {
        // do the values.yaml replace first.
        const proceed = await question_1.setupQuestionsFactory(questionGenerator)("Going to download the values.yaml file as helm-sumologic-values.yaml "
...
const installViaHelm = async (opts) => {
...
const verifyHelmInstall = (stdOpts) => {
...
exports.freshInstallHelm = freshInstallHelm;
//# sourceMappingURL=freshInstallHelm.js.map